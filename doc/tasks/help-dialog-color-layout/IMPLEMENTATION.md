...
Modify the help dialog's color palette rendering functions to display 2 colors per line instead of 6, preventing the dialog from stretching beyond its intended width.
...
- `renderColorCube()`: Changes loop structure from 6 columns to 2 columns
- `renderGrayscale()`: Changes loop structure from 6 columns to 2 columns
...
**Goal**: Display color cube (16-231) with 2 colors per line
...
|----------|---------|-------|
| `renderColorCube()` | 36 rows × 6 cols = 216 colors | 108 rows × 2 cols = 216 colors |

**Processing Flow**:
```
For each row (0 to 107):
  → Render 2 colors (row*2 + 16, row*2 + 17)
  → Append line to result
```

**Implementation Steps**:
1. Change outer loop from `row < 36` to `row < 108`
2. Change inner loop from `col < 6` to `col < 2`
3. Update color number calculation: `16 + row*2 + col`

**Testing**:
- Run existing tests to verify no regression
- Manual verification of output line count (should be 108 lines)
...
**Goal**: Display grayscale (232-255) with 2 colors per line
...
| Function | Current | After |
|----------|---------|-------|
| `renderGrayscale()` | 4 rows × 6 cols = 24 colors | 12 rows × 2 cols = 24 colors |

**Processing Flow**:
```
For each row (0 to 11):
  → Render 2 colors (row*2 + 232, row*2 + 233)
  → Append line to result
```

**Implementation Steps**:
1. Change outer loop from `row < 4` to `row < 12`
2. Change inner loop from `col < 6` to `col < 2`
3. Update color number calculation: `232 + row*2 + col`

**Testing**:
- Run existing tests to verify no regression
- Manual verification of output line count (should be 12 lines)
...
- **Line count verification**: Ensure all colors are still displayed
  - Mitigation: Verify 108 lines for cube, 12 lines for grayscale

## Performance Considerations

- More lines to render (108+12 vs 36+4), but negligible impact as content is pre-built once
