...
func (d *HelpDialog) buildContent() []string {
	...
	lines = append(lines, "  R              : rename (keep extension)")
	lines = append(lines, "  Shift+R        : rename (full name)")
	...
	lines = append(lines, "  !!             : execute shell command (background)")
	...
	lines = append(lines, "")
	lines = append(lines, "Background Command (during execution)")
	lines = append(lines, "  TAB            : focus output area")
	lines = append(lines, "  Ctrl+C (focus) : cancel background command")
	lines = append(lines, "  TAB/Esc (focus): return to file list")
	lines = append(lines, "")
	...
	lines = append(lines, "  Ctrl+G         : SQL-like filter")
	lines = append(lines, "")
	...
	lines = append(lines, "  B              : open bookmark manager")
	lines = append(lines, "  Shift+B        : add current directory to bookmarks")
	lines = append(lines, "")
	...
	// SQL-like Filter section
	lines = append(lines, "SQL-like Filter (Ctrl+G)")
	lines = append(lines, "")
	lines = append(lines, "Columns:")
	lines = append(lines, "  name, size, mtime, type, ext, perm")
	lines = append(lines, "  owner, group, isdir, isfile, issymlink")
	lines = append(lines, "")
	lines = append(lines, "Operators:")
	lines = append(lines, "  = != < > <= >=")
	lines = append(lines, "  LIKE ILIKE (% = any, _ = single char)")
	lines = append(lines, "  IN, NOT IN, IS NULL, IS NOT NULL")
	lines = append(lines, "  AND, OR, NOT")
	lines = append(lines, "")
	lines = append(lines, "Size Units:")
	lines = append(lines, "  Binary: KiB MiB GiB TiB")
	lines = append(lines, "  Decimal: KB MB GB TB")
	lines = append(lines, "")
	lines = append(lines, "Duration:")
	lines = append(lines, "  7d (days), 24h (hours), 30m (minutes)")
	lines = append(lines, "")
	lines = append(lines, "Functions:")
	lines = append(lines, "  now(), year(mtime), month(mtime), day(mtime)")
	lines = append(lines, "  lower(name), upper(name)")
	lines = append(lines, "")
	lines = append(lines, "Examples:")
	lines = append(lines, "  size > 1MiB")
	lines = append(lines, "  ext IN ('go', 'md')")
	lines = append(lines, "  name ILIKE '%.txt'")
	lines = append(lines, "  mtime > now() - 7d")
	lines = append(lines, "  isdir AND size > 100KiB")
	lines = append(lines, "")
	lines = append(lines, "")
